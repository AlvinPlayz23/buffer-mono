...
function loadProjectMemoryFile(cwd: string): { path: string; content: string } | null {
	const filePath = join(cwd, CONFIG_DIR_NAME, "memory.md");
	if (!existsSync(filePath)) {
		return null;
	}
	try {
...
function getMemoryAppendSystemPrompt(cwd: string): string | undefined {
	const memoryPath = join(cwd, CONFIG_DIR_NAME, "memory.md");
	if (!existsSync(memoryPath)) {
		return undefined;
	}
	return [
		`Memory mode is enabled. Maintain ${CONFIG_DIR_NAME}/memory.md using tools when stable facts change.`,
		"Keep memory concise and durable; avoid transient status text.",
	].join(" ");
}
...
function loadProjectContextFiles(
	...
	const memoryFile = loadProjectMemoryFile(resolvedCwd);
	if (memoryFile && !seenPaths.has(memoryFile.path)) {
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	async reload(): Promise<void> {
		...
		const memoryAppend = getMemoryAppendSystemPrompt(this.cwd);
		if (memoryAppend) {
			baseAppend.push(memoryAppend);
		}
