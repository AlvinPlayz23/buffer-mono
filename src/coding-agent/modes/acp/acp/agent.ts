...
import { BufferRpcProcess } from '../pi-rpc/process.js'
import { normalizePiAssistantText, normalizePiMessageText } from './translate/pi-messages.js'
import { promptToPiMessage } from './translate/prompt.js'
import { loadSlashCommands, parseCommandArgs, toAvailableCommands } from './slash-commands.js'
import { isAbsolute } from 'node:path'
import { existsSync, readFileSync, realpathSync, readdirSync, statSync } from 'node:fs'
import type { AvailableCommand } from '../sdk.js'
...
function builtinAvailableCommands(): AvailableCommand[] {
  return [
    ...
    {
      name: 'steering',
      description: 'Get/set buffer steering message delivery mode (how queued steering messages are delivered)',
      input: { hint: '(no args to show) all | one-at-a-time' }
    },
    {
      name: 'follow-up',
      description: 'Get/set buffer follow-up message delivery mode (how queued follow-up messages are delivered)',
      input: { hint: '(no args to show) all | one-at-a-time' }
    },
    {
      name: 'changelog',
      description: 'Show buffer changelog'
    },
    {
      name: 'help',
...
export class BufferAcpAgent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    return {
      ...
      agentInfo: {
        name: pkg.name ?? 'buffer-acp',
        title: 'buffer ACP adapter',
    ...
    if (attachments.length === 0 && message.trimStart().startsWith('/')) {
      ...
      if (cmd === 'changelog') {
        ...
        const findChangelog = (): string | null => {
          ...
          // On Node installs, `pi` typically resolves to .../buffer-agent/dist/cli.js
          try {
            const whichCmd = process.platform === 'win32' ? 'where' : 'which'
            const which = spawnSync(whichCmd, ['buffer'], { encoding: 'utf-8' })
          ...
          try {
            ...
            if (root) {
              const p = join(root, 'buffer-agent', 'CHANGELOG.md')
              if (existsSync(p)) return p
            }
          } catch {
            // ignore
        ...
        if (!changelogPath) {
          await this.conn.sessionUpdate({
            ...
            update: {
              sessionUpdate: 'agent_message_chunk',
              content: { type: 'text', text: "Changelog not found (couldn't locate buffer installation)." }
            }
          })
          return { stopReason: 'end_turn' }
        }
      ...
      if (cmd === 'view') {
        await this.conn.sessionUpdate({
          ...
          update: {
            ...
            content: {
              type: 'text',
              text: 'The /view command is interactive-only and is not supported in ACP mode.'
            }
          }
        })
        return { stopReason: 'end_turn' }
      ...
      if (cmd === 'export') {
        ...
        const outputPath = join(session.cwd, `buffer-session-${safeSessionId}.html`)
        ...
        if (!resultPath) {
          await this.conn.sessionUpdate({
            ...
            update: {
              ...
              content: {
                type: 'text',
                text: 'Export failed: no output path returned by buffer.'
              }
            }
          })
          return { stopReason: 'end_turn' }
        ...
        await this.conn.sessionUpdate({
          ...
          update: {
            ...
            content: {
              type: 'resource_link',
              name: `buffer-session-${safeSessionId}.html`,
              uri,
              mimeType: 'text/html',
  ...
  async loadSession(params: LoadSessionRequest): Promise<LoadSessionResponse> {
    ...
    const proc = await BufferRpcProcess.spawn({
      cwd: params.cwd,
...
async function getThinkingState(proc: BufferRpcProcess): Promise<{
...
async function getModelState(proc: BufferRpcProcess): Promise<{
...
function buildStartupInfo(opts: {
  ...
  try {
    ...
    const piVersion = spawnSync('buffer', ['--version'], { encoding: 'utf-8' })
    const installed = String(piVersion.stdout ?? '').trim().replace(/^v/i, '')
    if (installed) {
      md.push(`buffer v${installed}`)
      md.push('---')
      md.push('')
      ...
      try {
        const latestRes = spawnSync('npm', ['view', 'buffer-agent', 'version'], {
          encoding: 'utf-8',
          timeout: 800
        })
        ...
        if (latest && isSemver(latest) && isSemver(installed) && compareSemver(latest, installed) > 0) {
          updateNotice = `New version available: v${latest} (installed v${installed}). Run: \`npm i -g buffer-agent\``
        }
      } catch {
        // ignore
      }
  ...
  const skillsDir = join(process.env.HOME ?? '', '.buffer', 'agent', 'skills')
  try {
  ...
  const promptsDir = join(process.env.HOME ?? '', '.buffer', 'agent', 'prompts')
  try {
  ...
  const extDir = join(process.env.HOME ?? '', '.buffer', 'agent', 'extensions')
  try {
  ...
  // Also show npm packages from buffer settings (best-effort)
  try {
    const settingsPath = join(process.env.HOME ?? '', '.buffer', 'agent', 'settings.json')
