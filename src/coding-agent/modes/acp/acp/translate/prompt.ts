import type { ContentBlock } from '../../sdk.js'
...
export function promptToPiMessage(blocks: ContentBlock[]): {
  ...
  for (const b of blocks) {
    switch (b.type) {
      ...
      case 'audio': {
        // Not supported by ACP adapter runtime. Provide a marker so we don't silently drop context.
        const bytes = Buffer.byteLength(b.data, 'base64')
        message += `\n[Audio] (${b.mimeType}, ${bytes} bytes) not supported by buffer-acp`
        break
      }
