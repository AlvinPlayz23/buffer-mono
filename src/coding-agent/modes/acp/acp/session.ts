...
import { RequestError } from '../sdk.js'
...
export class SessionManager {
  private sessions = new Map<string, BufferAcpSession>()
  private readonly store = new SessionStore()

  /** Get a registered session if it exists (no throw). */
  maybeGet(sessionId: string): BufferAcpSession | undefined {
    return this.sessions.get(sessionId)
  }

  async create(params: SessionCreateParams): Promise<BufferAcpSession> {
    ...
    const session = new BufferAcpSession({
      sessionId,
      cwd: params.cwd,
  ...
  get(sessionId: string): BufferAcpSession {
   ...
   */
  getOrCreate(sessionId: string, params: SessionCreateParams & { proc: BufferRpcProcess }): BufferAcpSession {
...
export class BufferAcpSession {
  ...
  readonly proc: BufferRpcProcess
    ...
    const turnPromise = new Promise<StopReason>((resolve, reject) => {
      ...
      if (this.pendingTurn) {
        ...
        this.emit({
          ...
          _meta: { bufferAcp: { queueDepth: this.turnQueue.length, running: true } }
        })
  ...
  async cancel(): Promise<void> {
    ...
    if (this.turnQueue.length) {
      ...
      this.emit({
        sessionUpdate: 'session_info_update',
        _meta: { bufferAcp: { queueDepth: 0, running: Boolean(this.pendingTurn) } }
      })
    }
  ...
  private startTurn(t: QueuedTurn): void {
    ...
    this.proc.prompt(t.message, t.attachments).catch(err => {
      ...
      void this.flushEmits().finally(() => {
        ...
        this.emit({
          sessionUpdate: 'session_info_update',
          _meta: { bufferAcp: { queueDepth: this.turnQueue.length, running: false } }
        })
      })
      void err
    })
  ...
  private handlePiEvent(ev: BufferRpcEvent) {
      ...
      case 'agent_end': {
        ...
        void this.flushEmits().finally(() => {
          ...
          if (next) {
            ...
            this.emit({
              sessionUpdate: 'session_info_update',
              _meta: { bufferAcp: { queueDepth: 0, running: false } }
            })
          }
        })
        break
