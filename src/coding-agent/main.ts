...
import { type ImageContent, modelsAreEqual, supportsXhigh } from "#buffer-ai";
...
export async function main(args: string[]) {
	...
	if (parsed.acp) {
		...
		if (parsed.mode) conflicts.push("--mode");
		if (parsed.print) conflicts.push("--print");
		if (parsed.export) conflicts.push("--export");
		if (parsed.fileArgs.length > 0) conflicts.push("@file");
		if (parsed.messages.length > 0) conflicts.push("messages");

		if (conflicts.length > 0) {
			console.error(chalk.red(`Error: --acp cannot be combined with ${conflicts.join(", ")}`));
			process.exit(1);
		}
