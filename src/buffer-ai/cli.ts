...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx #buffer-ai <command> [provider]
...
Examples:
  npx #buffer-ai login              # interactive provider selection
  npx #buffer-ai login anthropic    # login to specific provider
  npx #buffer-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx #buffer-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx #buffer-ai --help' for usage`);
	process.exit(1);
}
